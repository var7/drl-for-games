...
if __name__ == '__main__':
    ...
    load_episode = range(1670, 2731,10)
    ...
    for episode in load_episode:
        print(episode)
        tf.train.Saver().restore(agent.session,'log/DQN/2018-03-12_11-50-04_SuperMarioAllStarsDeterministic-v4_False/episode_%d.ckpt'%(episode))
        c=0
        total_reward = 0.
        res = []
        while c<10:
            print(c)
            scores = agent.validate_episode(epsilon=0.05, visualise=True)
            res.append(scores)
            c+=1
            total_reward+=scores
